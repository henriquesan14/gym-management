...
public sealed class CreateMembershipCommandHandler(IUnitOfWork unitOfWork) : ICommandHandler<CreateMembershipCommand, Result>
{
    public async Task<Result> Handle(CreateMembershipCommand request, CancellationToken ct)
    {
        var member = await unitOfWork.Members.GetByIdAsync(MemberId.Of(request.MemberId), ct);
